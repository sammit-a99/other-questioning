...
demographics = {
    ...
    # "Asian Woman": f"A Asian woman who {BASE_BACKGROUND}",
    ...
    # "Amish man": f"A amish man who {BASE_BACKGROUND}",
...
# 3. Function to Query Gemini
def get_robot_questions(biography, num_trials=3):
    # system_prompt = (
...
def run_study():
    ...
    for group, bio in demographics.items():
        ...
        outputs = get_robot_questions(bio, num_trials=3)
    ...
    df.to_csv("demographic_bias_results_gemini.csv", index=False)
    print("\nData collection complete! Results saved to 'demographic_bias_results_gemini.csv'.")
